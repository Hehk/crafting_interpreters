...
type state = { tokens : Token.token list; current : int }
...
let createStatement tokens =
  ...
  let tokens =
    ...
    | _ -> raise (ParseError "Statements must end with a semicolon")
  in
...
let add x xs = match x with Some x -> x :: xs | None -> xs

let rec run ?(statements = []) ?(errors = []) tokens =
  let statement, tokens, error =
    try noError @@ createStatement tokens
    with ParseError message -> (None, syncronize tokens, Some message)
  in
  ...
  else run ~statements ~errors tokens
...
let%test "initial parse" =
  ...
  Statement.equal_statement statement
    (Statement.Expression
       (Expr.Binary
          ( Expr.Binary
              ( Expr.Literal (Expr.Num 5.),
                fakeTi Token.STAR,
                Expr.Literal (Expr.Num 4.) ),
            fakeTi EQUAL_EQUAL,
            Expr.Binary
              ( Expr.Literal (Expr.Num 2.),
                fakeTi PLUS,
...
let%test "synchronize, recover after semicolor" =
  let statements, _ =
    run
    @@ List.map ~f:fakeTi [ STAR; STAR; STAR; SEMICOLON; NUMBER 5.; SEMICOLON ]
  in
  let statement = List.hd_exn statements in
  Statement.equal_statement statement
    (Statement.Expression (Expr.Literal (Expr.Num 5.)))
...
let%test "synchronize, recover on var" =
  ...
  let statement = List.hd_exn statements in
  Statement.equal_statement statement
    (Statement.Variable ("nice", Expr.Literal (Expr.Num 69.)))
